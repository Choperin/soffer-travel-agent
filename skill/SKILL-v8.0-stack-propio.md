...
description: Agente de viajes premium familia Soffer — Stack Propio con Amadeus, Perplexity API, LangGraph, n8n
metadata:
  version: '8.0-sp'
...
> **Versión STACK PROPIO (v8.0-sp)**: Esta versión incluye integraciones con Amadeus Self-Service APIs, Perplexity Search API, Browserless, LangGraph multi-agente y n8n para automatización. Para la versión BASE que funciona con Perplexity Computer, ver SKILL.md v8.0.
...
Cuando realmente haga falta ir fuera del skill y de Supabase, respetar este orden de menor a mayor costo y complejidad:

1. **APIs estructuradas** (cuando existan): Amadeus Self-Service y otras APIs autorizadas en el stack propio.
2. **Perplexity Search API** para contexto y respuestas sintetizadas desde múltiples fuentes, evitando navegación masiva.
3. **Navegación controlada (Browserless)** solo cuando:
   - No haya API disponible para la tarea, y
   - Se requiera leer una página MUY específica (política de aerolínea, detalle de hotel, etc.).

Antes de usar Browserless:
- [ ] ¿Puedo resolver esto con Amadeus u otra API autorizada?
...
### S35 — Stack Propio: APIs Externas (v8.0-sp)

**Amadeus Self-Service APIs** (vuelos, hoteles):
- Wrapper: `searchFlightsAmadeus(params)`, `searchHotelsAmadeus(params)`
- Normalizar respuestas a formato común interno
- Guardar resultados en `research_cache` con expiración (6h vuelos, 24h hoteles)
- Respetar S0.6: usar Amadeus ANTES de Perplexity o Browserless

**Perplexity Search API** (contexto, visas, clima, destinos):
- Wrapper: `searchContextPerplexity(prompt)`
- Construir prompts estructurados (requisitos visa, clima, contexto de destino)
- Guardar extractos útiles en `research_cache`
- Usar cuando no haya API estructurada para el tema

**Browserless** (navegación controlada):
- Wrapper: `browserAction(plan)`
- Solo cuando no exista API ni Perplexity sea suficiente
- Limitar a pocas URLs específicas, sin navegación masiva
- Respetar S0.6: es el último recurso

**Backend helpers (Supabase):**
- `getTripBySlugOrDestination` — Resuelve `trip_id`
- `getActiveTrip` — Viaje actual por `trip_status` activo
...
- `upsertReservations(trip_id, items)` — Crea/actualiza reservas
- `getOrSetResearchCache(params)` — Consulta cache, si no hay datos usa servicios externos

Estas funciones son la **única forma** de acceder a la BD desde el resto del backend.

### Orquestación Multi-Agente con LangGraph (v8.0-sp)

Grafo de agentes que usa las funciones del backend como building blocks:

| Agente | Responsabilidad |
|---|---|
| **PlannerAgent** | Interpreta solicitud del usuario, aplica S0 (S0.1, S0.4, S0.6), decide qué tareas A–F ejecutar |
| **FlightsAgent** | Vuelos usando Amadeus + cache, escribe en `research_cache` y `reservations` |
| **HotelsAgent** | Hoteles usando Amadeus + cache |
| **ActivitiesAgent** | Actividades usando Perplexity + cache |
| **RestaurantsAgent** | Restaurantes usando Perplexity + cache |
| **MonitoringAgent** | Observa `trip_status`, agenda tareas de monitoreo, actualiza estados según fechas |

**Estado compartido del grafo:**
- `trip_id` y snapshot de `trips`, `reservations`, `travelers`, `documents`
- Nivel de modelo (LIGERO / ESTÁNDAR / PREMIUM) según S45
- Flags de autonomía (S5), kosher (S3) y prioridad de reservas (S6a)

Cada agente debe **leer primero** INDEX/DATA/Supabase/cache (S0.4 + S41b) y solo después usar APIs externas o navegador (S0.6).
...
|---|---|---|
| **LIGERO** | `gemini_3_flash` | Tareas mecánicas, repetitivas, sin análisis profundo |
| **ESTÁNDAR** | `claude_sonnet_4_5` | Análisis, comparación, redacción con contexto |
| **PREMIUM** | `claude_sonnet_4_6` o `gemini_3_1_pro` | Creatividad compleja, planificación multi-destino, bocetos |

### Asignación por Tarea

| Tarea | Nivel |
|---|---|
| Lectura/clasificación emails, escaneo créditos (S15), crear eventos Calendar, monitoreo precios, actualizar Sheets, refresh mensual beneficios tarjetas (S46 Ciclo 2) | LIGERO |
| Cotización Modo Rápido (S22), búsqueda vuelos/hoteles, redacción correos a proveedores, correo diario (S29), alertas Nivel 1 con alternativas (S28), evaluación millas (S34), investigación actividades/hidden gems, auditoría trimestral tarjetas (S46 Ciclo 3) | ESTÁNDAR |
| Boceto Estratégico Premium (S23), generación HTML (boceto/portal), planificación multi-destino (>2 países), itinerario definitivo (S23), auto-actualización mensual (S37), protocolo emergencia/evacuación (S10), post-trip review y ajustes al skill (S44) | PREMIUM |

### Reglas de Aplicación
1. **Sub-agentes**: Heredan el nivel de la tarea que los invoca (búsqueda = ESTÁNDAR, síntesis final = PREMIUM).
2. **browser_task**: LIGERO por default; ESTÁNDAR si requiere análisis complejo.
3. **Crons**: LIGERO salvo auto-actualización mensual (PREMIUM).
4. **Auto-escalamiento**: LIGERO falla → ESTÁNDAR. ESTÁNDAR falla en tarea creativa → PREMIUM. Máximo 1 escalamiento.
5. **Excepción `trip_status = 'en_viaje'`**: Todas las tareas escalan a ESTÁNDAR mínimo; emergencias Nivel 1 → PREMIUM directo.
6. **Excepción usuario**: Si Moisés pide "calidad máxima" o "mejor modelo" → PREMIUM.
